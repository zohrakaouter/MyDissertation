...
Due to metamodel evolution, the model become "no conform"
%Once the metamodel changes are detected, using one of the previously presented approaches in Section ?, 
 ...
 Co-evolution  approaches use automatic or manyal diffing metamodel approaches in their solution. Model co-evolution apporaches that we found can be categorized into four categories. The first category is resolution strategy languages that specifies in a transformation laguage how to update the model given the list of metamodel changes. The category REsolution strategy generation groups approaches that generate full or partial resolution for each metamodel change.
 The co-evolution between metamodel and models can be processed manually \cite{bibid} but it require a huge expertise. We find three categories of metamodel and model co-evolution approaches.  Approaches based on resolution strategy languages that propse transformation languages created given the metamodel changes. The second categorie groups Resolution strategy generation approaches that allow to generate full or partial resolutions for each metamodel change. Some of these approaches  rzquires the user intervention to make decision on the selected operation to adapt the model. The gategory of  Resoltion strategy leaning that adopt machine learning algorithm to sekect the resolution stratefy for metamodel changes.
. Other approaches consider the model co-evolution problem as an opitimization one that does not need the list of changes of the metamodel \cite{}.
...
Almost all the exixsting transformation approaches that we find in literratue starts by analysing the impact of the metamodel on the model transformations.
Mendez et al. [30] and Ruscio et al. [8,Garces et al. [10],Kusel, et al. [29] and khelladi proposed to co-evolve impacted transformation with a set of resolutions, khelladi covers the larget set of possible resolutions where Ruscio et al. [8, estilate the cost of the co-evoltuion to decide about the co-evolution,they further explored the variability of the co-evolution due to the possible alternative resolution . Garcia et al. [11] propsed a atl tranformation-base approach wich means that their resolutions are ATL transformations. All the approaches propse unique resolution to co-evolve the transformation
Ruscio et al. [8 allow developers to manually replace or refine a resolution. Khelladi et al allows to compose existing resolutions into a new one. Kessentini et all used a different approach that do not use the changes of the metamodel as inout and do not process by an impact ananlysis, but it uses a search-based approahc that relied on multi objective heuristic algorithm NSGA-II.
...
%Discussion about automation degree of the approaches
